...
for s in soup: img_links.add("http:" + s["src"])
...
for s in soup: 
    img_link = "http:" + s["src"]
    if 'anner' in img_link: img_links.add(img_link)
...
for i, link in enumerate(img_links):
    img_data = Image.open(requests.get(link, stream = True, verify = True).raw).convert("RGB")
    img_data.save(path + "osgo_" + str(i+1) + ".jpg", "JPEG")

# record
osgo_df = pd.DataFrame()
osgo_df['banner_source'] = list(img_links)
